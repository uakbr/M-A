...
1. **Balance Sheets**:
   - Prepare the acquirer’s and target’s balance sheets in Excel or CSV format.
   - Ensure consistent formats (e.g., rows for line items, columns for periods).

2. **Scenarios**:
   - Create a JSON file (`scenarios.json`) to store assumptions for Base Case, Optimistic, and Pessimistic scenarios.
   - Example structure:
     ```json
     {
         "base_case": {
             ...
             "synergies": 0.1,
             "financing_mix": {"equity": 0.5, "debt": 0.5}
         },
         "optimistic": {
             "purchase_price": 95000000,
             "synergies": 0.15,
             "financing_mix": {"equity": 0.4, "debt": 0.6}
         },
         "pessimistic": {
             "purchase_price": 105000000,
             "synergies": 0.05,
             "financing_mix": {"equity": 0.6, "debt": 0.4}
         }
     }
     ```

3. **Synergy Inputs**:
   - Include cost and revenue synergies (e.g., percentage savings, revenue growth assumptions).
...
2. Update input files in the `data/` directory.

3. Run the main script for initial outputs:
   ```bash
   python main.py
   ```
